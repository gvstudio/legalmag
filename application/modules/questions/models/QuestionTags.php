...
class Questions_Model_QuestionTags extends Rastor_Model_TableAbstract {
    ...
    public function getQuestionTagsLinksList($id){
        $records = $this->getDbTable()->getQuestionTags($id);
                
        $tags = array();
        foreach($records as $record) {
            $tags[] = '<a href="'.Rastor_Url::get('question_tag', array('tag' => $record->id)).'">'.$record->name.'</a>';
        }
