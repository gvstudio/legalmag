 ...
 * Rastor CMS
 * @author Budjak Orest
 * @copyright 2011-2012 Budjak Orest
...
jQuery.fn.RastorPictures = function(ops){
        ...
        rebuild: function(){
            ...
            for (var i = 0; i < this.data.length; i++){
                ...
                if ((this.data[i].picture !== undefined) && (this.data[i].thumb !== undefined)){
                    $('#' + this.id).append('<li indx="' + i + '">' + linksTop + '<div class="holder"><table><tr><td style="width: ' + options.previewWidth + 'px; height: ' + options.previewHeight + 'px; text-align: center; vertical-align: middle;"><img src="' + thumb + '" alt="" /></td></tr></table></div><div class="links">' + links + '</div></li>');
                } else {
    ...
    var pictureObj = {
        ...
        init: function(){
            ...
            this.id = getNewId('picture_holder');
            this.addId = getNewId('add_photo');
        ...
        rebuild: function(){
            ...
            if ((this.data.picture !== undefined) && (this.data.thumb !== undefined) && (this.data.thumb.length)){
                $('#' + this.id).append('<li><div class="holder"><table><tr><td style="width: ' + options.previewWidth + 'px; height: ' + options.previewHeight + 'px; text-align: center; vertical-align: middle;"><img src="' + thumb + '" alt="" /></td></tr></table></div><div class="links">' + links + '</div></li>').disableSelection();
        ...
        remove: function(){
            ...
            $('#dialog').dialog({
                ...
                buttons: [
                {
                    text: options.translation.buttonYes,
                    click: function() {
                        deleteFile(pObj.data.picture);
                        deleteFile(pObj.data.thumb);
                        pObj.data = {};
                        pObj.rebuild();
                        $(this).dialog('close');
                    }
    ...
    function updatePreviewFixed(c){        
        if (parseInt(c.w) > 0)
        {
            var rx = options.previewWidth / c.w;
            var ry = options.previewHeight / c.h;
            ...
            picture = filename;
            request = filename+':'+Math.round(imgRateX * c.x)+':'+Math.round(imgRateX * c.y)+':'+Math.round(imgRateX * c.w)+':'+Math.round(imgRateX * c.h)+ ':' + options.previewWidth + ':' + options.previewHeight;
        }
    }
    
    function updatePreview(c){ 
        if ((parseInt(c.w) > 0) && (parseInt(c.h) > 0)) {
            if (c.w / c.h > options.previewWidth / options.previewHeight){
                newWidth = options.previewWidth;
                newHeight = Math.round(options.previewWidth * c.h / c.w);
                $('#preview_holder').css({
                    width: newWidth + 'px',
                    height: newHeight + 'px',
                    left: '0px',
                    top: Math.round((options.previewHeight - (options.previewWidth * c.h / c.w)) / 2)+'px'
                });
                rx = options.previewWidth / c.w;
                ry = newHeight / c.h;
            } else {
                newWidth = Math.round(options.previewHeight * c.w / c.h);
                newHeight = options.previewHeight;
                $('#preview_holder').css({
                    width: newWidth + 'px',
                    height: newHeight + 'px',
                    top: '0px',
                    left: Math.round((options.previewWidth - (options.previewHeight * c.w / c.h)) / 2)+'px'
                });
                rx = newWidth / c.w;
                ry = options.previewHeight / c.h;
            }
    ...
    function setBlocksSizes(res){
        max_width = window.innerWidth - options.previewWidth - 100;
        ...
        if ((res.width > max_width) && (res.height > max_height)){
            if ((max_width / res.width) > (max_height / res.height)){
                ...
                $('#dialog').dialog({
                    width: ($('#target').width() + options.previewWidth + 30),
            ...
            var dheight = ($('#target').height() + options.previewWidth + 30);
            if (dheight < 200 + options.previewHeight){
                dheight = 200 + options.previewHeight;
            }
            ...
            var dwidth = ($('#target').width() + options.previewWidth + 30);
            if (dwidth < 200 + options.previewWidth){
                dwidth = 200 + options.previewWidth;
            }
    ...
    function setStartRectFixed(response){
        ...
        imgRateXb = boundx / options.previewWidth;
        imgRateYb = boundy / options.previewHeight;
        if (options.previewHeight * imgRateXb > $('#target').height()) {
            newWidth = options.previewWidth * imgRateYb;
            newHeight = boundy;
            posX = (boundx - newWidth) / 2;
            posY = 0;
        } else {
            newWidth = boundx;
            newHeight = options.previewHeight * imgRateXb;
                    ...
                    if (response.error){
                        ...
                        if (options.crop){
                            if (options.fixedThumbSize){
                                $('#dialog').html('<img id="target" src="'+response.filename+'" alt="" /><div style="width:'+options.previewWidth+'px;height:'+options.previewHeight+'px;overflow:hidden; position: absolute; top: 6px; right: 10px;border: 1px solid #bbb;"><img src="' + response.filename + '" id="preview_pict" alt="" /></div>');
                                ...
                                $('#dialog').html('<img id="target" src="'+response.filename+'" alt="" /><div style="width:'+options.previewWidth+'px;height:'+options.previewHeight+'px;overflow:hidden; position: absolute; top: 6px; right: 10px; border: 1px solid #bbb;"><div id="preview_holder" style="position: absolute; overflow: hidden;"><img src="' + response.filename + '" id="preview_pict" alt="" /></div></div>');
                                setBlocksSizes(response);
                                buildEditCropDialog(index);
                                $('#target').Jcrop({
    ...
    function callback(){
        callback = $.parseJSON(options.callback);
